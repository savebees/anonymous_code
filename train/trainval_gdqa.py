...
def eval(model, data_loader, a2v, args, test=False):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(data_loader):
            ...
            # break
            ###################################
            fusion_proj, answer_proj, gauss_params, kargs, div_loss, sim_loss = model( #新增gauss_params
                video_frames,
                video_mask,
                question,
                question_mask,
            ...
            gauss_params = gauss_params.squeeze().cpu().numpy() 
            ...
            for bs, qid in enumerate(question_id):
                ...
                if args.baseline == 'posthoc':
                    ...
                    ground_res[qid] = att[bs].tolist()
                    #新增存储gauss_params，在eval 时候用mgqa找span
                    results[qid]['gauss_params'] = gauss_params[bs].tolist()
...
def train(model, train_loader, a2v, optimizer, criterion, scheduler, epoch, args, tokenizer):
    model.train()
    running_vqa_loss, running_acc, running_vg_loss, running_div_loss, running_sim_loss = (
        AverageMeter(),
        AverageMeter(),
        AverageMeter(),
        AverageMeter(),
    ...
    for i, batch in enumerate(train_loader):
        ...
        if args.baseline in ['NG+', 'posthoc', 'NG']:
            ...
            fusion_proj, answer_proj, gauss_params, args_vg, div_loss, sim_loss = model( #修改这里
                video_frames,
                video_mask,
                question,
                question_mask,
        ...
        # MGQA all loss
        loss = vqa_loss + args.div_loss_weight * div_loss + args.sim_loss_weight * sim_loss  
        ...
        running_sim_loss.update(sim_loss.detach().cpu().item(), N) #新增
        ...
        if (i + 1) % (len(train_loader) // args.freq_display) == 0:
            ...
            if args.vg_loss and not args.div_loss:
                ...
                if not args.vg_loss and args.div_loss:
                    ...
                    logging.info(
                        f"Epoch {epoch + 1}/{args.epochs}, Progress: {float(i + 1) / len(train_loader):.4f}, Lvqa loss: "
                        f"{running_vqa_loss.avg:.4f}, Train acc: {running_acc.avg:.2%}, Sim Loss: {running_sim_loss.avg:.4f}"
                    ) 
            ...
            if args.div_loss:
                running_div_loss.reset()
            running_sim_loss.reset() #add